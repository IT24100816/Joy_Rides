...
<head>
  ...
  <title>JOY-RIDE - Rent Details</title>
...
<div class="content">
  ...
  <div class="welcome-message">
    <h1>Welcome to Rent Details!</h1>
    <% String username = (String) session.getAttribute("username"); %>
    <p>Hi<%= username != null ? ", " + username : "" %>! Manage rental requests here.</p>
  </div>

  <h2 class="text-3xl font-bold mb-6">Rental Requests</h2>

  <%
    String RENTAL_REQUESTS_FILE_PATH = "/Users/samadhithjayasena/Library/CloudStorage/OneDrive-SriLankaInstituteofInformationTechnology/IntelliJ IDEA/Website/src/main/resources/RentalRequests.txt";

    // List to store rental request details
    List<String[]> rentalRequests = new ArrayList<>();
    try (BufferedReader requestReader = new BufferedReader(new FileReader(RENTAL_REQUESTS_FILE_PATH))) {
      String line;
      while ((line = requestReader.readLine()) != null) {
        if (line.trim().isEmpty()) continue;
        String[] parts = line.split(" \\| ");
        if (parts.length >= 5) {
          // Extract fields after removing labels
          rentalRequests.add(new String[]{
                  parts[0].replace("Bike Name: ", "").trim(),
                  parts[1].replace("Username: ", "").trim(),
                  parts[2].replace("Order Number: ", "").trim(),
                  parts[3].replace("Rental Days: ", "").trim(),
                  parts[4].replace("Additional Services: ", "").trim()
          });
        } else {
  %>
  <div class="alert alert-warning" role="alert">
    Warning: Skipping invalid rental request entry: <%= line %>
  </div>
  <%          continue;
  }
  }
  ...
  %>
  <div class="alert alert-danger" role="alert">Error loading rental request data: <%= e.getMessage() %></div>
  <% return; } %>
  ...
  <div class="overflow-x-auto">
    <table class="custom-table w-full">
      ...
      <tr>
        <td colspan="6" class="text-center">No rental requests found.</td>
      </tr>
      <% } else { %>
      <% for (String[] rental : rentalRequests) { %>
      <tr>
        <td><%= rental[0] != null ? rental[0] : "N/A" %></td>
        <td><%= rental[1] != null ? rental[1] : "N/A" %></td>
        <td><%= rental[2] != null ? rental[2] : "N/A" %></td>
        <td><%= rental[3] != null ? rental[3] : "N/A" %></td>
        <td><%= rental[4] != null ? rental[4] : "N/A" %></td>
        <td>
          <a href="AdminUpdateRent.jsp?bikeName=<%= URLEncoder.encode(rental[0], StandardCharsets.UTF_8) %>&username=<%= URLEncoder.encode(rental[1], StandardCharsets.UTF_8) %>" class="btn btn-primary btn-custom text-white mr-2">Update</a>
          <a href="DeleteRentalRequestServlet?orderNumber=<%= URLEncoder.encode(rental[2], StandardCharsets.UTF_8) %>" class="btn btn-danger btn-custom text-white" onclick="return confirm('Are you sure you want to delete this rental request?');">Delete</a>
        </td>
      </tr>
      <% } %>
      <% } %>
